...
class Bicicleta(models.Model):
    ...
    modadlidad = models.CharField(max_length=500, blank=True)
    ...
    def __str__(self):
        return f"{self.modelo} - {self.marca} - {self.codigo} - {self.modadlidad} - {self.n_serie} - {self.tienda_origen} - {self.factura} - {self.color}"
